...
if [ ! -f "../certs/test_cert_sm2.crt" ]; then
    echo "❌ Файл ../certs/test_cert_sm2.crt не найден"
    exit 1
fi

if [ ! -f "../certs/test_cert_sm2.key" ]; then
    echo "❌ Файл ../certs/test_cert_sm2.key не найден"
    exit 1
fi
...
if openssl x509 -in ../certs/test_cert_sm2.crt -checkend 0 -noout > /dev/null 2>&1; then
